...
struct FriendLocation: Identifiable, Codable {
    ...
    var pathCoordinates: [CodableCoordinate]
    var pathColor: String
    
    init(friendId: String, friendName: String, friendCode: String, coordinate: CLLocationCoordinate2D, pathColor: String = "orange") {
        self.id = UUID()
        ...
        self.pathCoordinates = [CodableCoordinate(coordinate)]
...
class LiveLocationManager {
    ...
    private func setupSimulatedFriends() {
        ...
        let simulatedFriends: [(String, String, FriendStatus, String, String)] = [
            ("Priya", "12345678", .studying, "At the library", "purple"),
            ("Rahul", "23456789", .exploring, "Walking near sports complex", "orange"),
            ("Ananya", "34567890", .eating, "At food court", "pink"),
            ("Vikram", "45678901", .exercising, "At gym", "cyan"),
            ("Sneha", "56789012", .inClass, "Engineering Block", "red")
        ]
        
        for (_, (name, code, status, activity, color)) in simulatedFriends.enumerated() {
            ...
            // Generate sample path coordinates for each friend
            var pathCoords: [CodableCoordinate] = []
            var currentLat = startCoord.latitude
            var currentLon = startCoord.longitude
            
            for _ in 0..<20 {
                currentLat += Double.random(in: -0.0003...0.0003)
                currentLon += Double.random(in: -0.0003...0.0003)
                pathCoords.append(CodableCoordinate(CLLocationCoordinate2D(latitude: currentLat, longitude: currentLon)))
            }
            
            friend.pathCoordinates = pathCoords
    ...
    func simulateFriendMovement() {
        for i in friendLocations.indices {
            ...
            friendLocations[i].coordinate = CodableCoordinate(newCoord)
            friendLocations[i].lastUpdated = Date()
            
            // Append to path (keep last 50 points)
            friendLocations[i].pathCoordinates.append(CodableCoordinate(newCoord))
            if friendLocations[i].pathCoordinates.count > 50 {
                friendLocations[i].pathCoordinates.removeFirst()
            }
