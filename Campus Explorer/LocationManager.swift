...
class LocationManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    @Published var userWeight: Double = 70.0 // kg, can be customized
    ...
    private let caloriesKey = "caloriesBurned"
    private let userWeightKey = "userWeight"
    ...
    func startTracking() {
        // NOTE: Background location updates require the "Location updates" 
        // background mode capability in Info.plist. If you need background
        // tracking, enable it in Xcode: Target > Signing & Capabilities > 
    ...
    private func calculateCalories() {
        ...
        // Approximate: 0.04 calories per step for average person
        // More accurate: calories = steps * 0.04 * (weight / 70)
        let caloriesPerStep = 0.04 * (userWeight / 70.0)
        caloriesBurned = Double(stepCount) * caloriesPerStep
        saveCalories()
    }
    
    private func saveCalories() {
        UserDefaults.standard.set(caloriesBurned, forKey: caloriesKey)
    }
    
    private func loadCalories() {
        caloriesBurned = UserDefaults.standard.double(forKey: caloriesKey)
    }
    
    func setUserWeight(_ weight: Double) {
        userWeight = weight
        UserDefaults.standard.set(weight, forKey: userWeightKey)
        calculateCalories() // Recalculate with new weight
    }
    
    private func loadUserWeight() {
        let saved = UserDefaults.standard.double(forKey: userWeightKey)
        if saved > 0 {
            userWeight = saved
        }
    }
    ...
    func resetPath() {
        ...
        UserDefaults.standard.removeObject(forKey: caloriesKey)
