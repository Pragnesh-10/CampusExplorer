...
struct FriendsView: View {
        ...
        NavigationView {
            List {
                ...
                Section("Connect with Friend") {
                    ...
                    if showConnected {
                        Text("Friend connected successfully!")
                            .font(.caption)
                            .foregroundStyle(.green)
                    }
                }
            ...
            .onChange(of: showConnected) { _, newValue in
                if newValue {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                        showConnected = false
                    }
    ...
    private func connectWithFriend() {
        ...
        guard friendCode.count == 8 else {
            errorMessage = "Code must be 8 characters"
            showError = true
            return
        }
        ...
        if authManager.connectWithFriend(friendCode: friendCode) {
            ...
            } else if authManager.currentUser?.friendIds.contains(friendCode) == true {
                errorMessage = "Already connected with this friend"
            } else {
                errorMessage = "Invalid code or connection failed"
            }
            showError = true
        }
    }
